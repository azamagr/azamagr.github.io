 ...
 *
 *  - Background: your "video.png" photo shown at full clarity — no blur,
 *    no dimming. A gradient sits only on the left edge (where the text
 *    is) so copy stays readable without darkening the whole photo.
 *  - Left: heading + subheading + two CTAs, all left-aligned.
...
export default function Hero({ bgImageSrc = "/IMG/video.png" }) {
  return (
    ...
    >
      {/* Background photo, shown at full clarity */}
      <img
        src={bgImageSrc}
        alt=""
        className="absolute inset-0 h-full w-full object-cover"
      />
      {/* Gradient only on the left/bottom edge, so the text stays legible
          without dimming the rest of the photo */}
      <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/30 to-transparent sm:from-black/75 sm:via-black/10 sm:to-transparent" />
      <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-transparent to-transparent" />

      {/* Content */}
      <div className="relative z-20 mx-auto flex h-full w-full max-w-7xl items-center px-6 sm:px-10">
        <motion.div
