  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto mb-24 max-w-3xl px-6 text-center sm:px-10">
        <span className="inline-block rounded-full border border-black/10 bg-black/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.2em] text-black/70">
          How I Work
        </span>
  ...
  return (
    ...
    >
      <motion.div
        ref={ref}
        animate={{
          backgroundColor: inView ? "#ff2a2a" : "#ffffff",
          boxShadow: inView
            ? "0 0 50px rgba(255,42,42,0.55), 0 20px 40px rgba(0,0,0,0.15)"
            : "0 10px 30px rgba(0,0,0,0.08)",
        }}
        transition={{ duration: 0.4, ease: "easeOut" }}
        className="relative w-[85%] max-w-xs rounded-2xl border border-black/10 px-6 pb-6 pt-5 sm:max-w-sm"
      >
        {/* Hole-punch detail, like a luggage tag / badge */}
        <div
          className={`absolute -top-3 h-6 w-6 rounded-full border-4 border-white ${
            align === "left" ? "left-6" : "right-6"
          }`}
          style={{ backgroundColor: inView ? "#ff2a2a" : "#e5e5e5" }}
        />

        <span
          className={`text-xs font-black tracking-[0.2em] transition-colors duration-300 ${
            inView ? "text-white/70" : "text-black/40"
          }`}
        >
          {id}
        </span>
        ...
        <p
          className={`mt-2 text-sm leading-relaxed transition-colors duration-300 ${
            inView ? "text-white/90" : "text-black/60"
          }`}
        >
          {desc}
        </p>
